...
#directorio del archivo en donde esta el proyecto
path = "C:/Users/alexc/Documents/10semestre/mtra.yanet/proyectofinal"
...
# REGISTER #
def register_face(photo):
    """
    Registra la foto de un usuario en la base de datos.

    Args:
    - photo_path (str): Ruta de la foto del usuario en el sistema de archivos.
    """
    name = os.path.basename(photo).split('.')[0]  # Extraer el nombre de archivo sin extensión
    result = db.save_user_photo(name, photo)  # Usar 'photo' en lugar de 'photo_path'
...
def register_capture():
    ...
    for i, face_data in enumerate(faces):
        ...
        face_img = pixels[y1:y1+height, x1:x1+width]
        cv2.imwrite(f"{img}_{i}.jpg", cv2.cvtColor(face_img, cv2.COLOR_RGB2BGR))
        register_face(img)
